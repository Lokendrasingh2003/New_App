...
import { SearchAllResult, SearchSuggestionType, searchAll } from '../../services/search/searchService';
...
export function AppHeader({
  ...
  const [localSearchResult, setLocalSearchResult] = useState<SearchAllResult>({
    shops: [],
    products: [],
    categories: [],
    suggestions: [],
  ...
  const [isShopkeeperUser, setIsShopkeeperUser] = useState(false);
  ...
  useEffect(() => {
    ...
    const runSearch = async () => {
      ...
      const result = await searchAll(activeQuery);
      if (!cancelled) {
  ...
  const refreshHeaderMeta = useCallback(async () => {
    if (!isAuthenticated) {
      setIsShopkeeperUser(false);
    }

    await seedNotificationsIfEmpty();
    const [count, prefs, draft, submissions, profile] = await Promise.all([
      getUnreadCount(),
      getNearbyPrefs(),
      getDraft(),
      getSubmissions(),
    ...
    const canManageShop = Boolean(profile && (profile.role === 'SHOPKEEPER' || profile.canManageSellerDashboard));
    setIsShopkeeperUser(canManageShop);
  }, [isAuthenticated]);
  ...
  const handleSellerPress = () => {
    ...
    if (isShopkeeperUser) {
      ...
      Linking.canOpenURL(dashboardUrl)
        .then((canOpen) => {
          if (!canOpen) {
        ...
        <View style={styles.iconActionsRow}>
          <Pressable style={styles.sellerButton} onPress={handleSellerPress}>
            <AppText style={styles.sellerButtonText}>
              {isShopkeeperUser ? 'Manage your shop' : hasSellerProgress ? '🏪 Status' : '🏪 Register'}
            </AppText>
          </Pressable>
