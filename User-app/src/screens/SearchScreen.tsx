...
import { MockSearchProduct, SearchAllResult, SearchSuggestionType, searchAll } from '../services/search/searchService';
...
export function SearchScreen() {
  ...
  const [searchResult, setSearchResult] = useState<SearchAllResult>({
    shops: [],
    products: [],
    categories: [],
    suggestions: [],
  ...
  useEffect(() => {
    ...
    const hydrate = async () => {
      ...
      try {
        const result = await searchAll(trimmedQuery);
        if (!cancelled) {
