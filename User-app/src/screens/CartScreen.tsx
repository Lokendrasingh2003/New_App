...
export function CartScreen() {
  ...
  const [availableCoupons, setAvailableCoupons] = useState<
    Array<{ id: string; title: string; code: string; description: string; minOrderValue?: number; expiryDate: string }>
  >([]);
  ...
  useEffect(() => {
    const loadAvailableCoupons = async () => {
      ...
      try {
        ...
        const mapped = (response.coupons || []).map((coupon) => {
          const title =
          ...
          return {
            ...
            title,
            description: String(coupon.description || title),
  ...
  const displaySavings = Math.max(totalSavingsAmount || savings + discountAmount, 0);
        ...
        <>
          ...
          >
            ...
            <View style={styles.billCard}>
              ...
              <View style={styles.billRow}>
                <AppText style={styles.billLabel}>Selling price total</AppText>
                <AppText style={styles.billValue}>₹{Math.max(subtotal, 0)}</AppText>
              </View>
              <View style={styles.billRow}>
                <AppText style={styles.billLabel}>Product savings</AppText>
                <AppText style={styles.savingValue}>-₹{Math.max(savings, 0)}</AppText>
              </View>
              {offerDiscountAmount > 0 ? (
                <View style={styles.billRow}>
                  <AppText style={styles.couponDiscountLabel}>Offer discount</AppText>
                  <AppText style={styles.couponDiscountValue}>-₹{Math.max(offerDiscountAmount, 0)}</AppText>
                </View>
              ) : null}
              <View style={styles.billRow}>
              ...
              {appliedCoupon || couponDiscountAmount > 0 ? (
                <View style={styles.billRow}>
                  ...
                  <AppText style={styles.couponDiscountValue}>
                    -₹{Math.max(couponDiscountAmount || discountAmount, 0)}
                  </AppText>
                </View>
              ) : null}
              {totalDiscountAmount > 0 ? (
                <View style={styles.billRow}>
                  <AppText style={styles.couponDiscountLabel}>Total discount</AppText>
                  <AppText style={styles.couponDiscountValue}>-₹{Math.max(totalDiscountAmount, 0)}</AppText>
                </View>
              ) : null}
              <View style={styles.billRow}>
                <AppText style={styles.billLabel}>Taxes & charges</AppText>
                <AppText style={styles.billValue}>₹{Math.max(taxAmount, 0)}</AppText>
              </View>
              <View style={styles.billRow}>
                <AppText style={styles.savingLabel}>Total savings</AppText>
              ...
              <Pressable style={styles.modalSheet} onPress={() => undefined}>
                ...
                <ScrollView showsVerticalScrollIndicator={false}>
                  {availableCoupons.length === 0 ? (
                    <AppText style={styles.noCouponsText}>No coupons available for this shop right now.</AppText>
                  ) : null}
