...
export function SellerSubmissionDetailsScreen() {
  ...
  const categoryLabel = useMemo(() => submission?.categoryName || '--', [submission?.categoryName]);
    ...
    <Screen>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.contentWrap}>
        <View style={styles.card}>
          ...
          <AppText style={styles.lineText}>Category: {categoryLabel}</AppText>
          <AppText style={styles.lineText}>Phone: {submission.phone || '--'}</AppText>
          <AppText style={styles.lineText}>Opening: {submission.openingTime || '--'}</AppText>
          <AppText style={styles.lineText}>Closing: {submission.closingTime || '--'}</AppText>
          <AppText style={styles.lineText}>Image: {submission.shopImageUrl || '--'}</AppText>
          {submission.shopImageUrl ? (
            <Image source={{ uri: submission.shopImageUrl }} style={styles.previewImage} resizeMode="cover" />
          ) : null}
          ...
          {submission.rejectionReason ? (
            <AppText style={styles.lineText}>Rejection: {submission.rejectionReason}</AppText>
          ) : null}
        </View>
        ...
        <View style={styles.card}>
          ...
          {submission.documents.businessProofUri ? (
            <Image source={{ uri: submission.documents.businessProofUri }} style={styles.previewImage} resizeMode="cover" />
          ) : null}
          <AppText style={styles.lineText}>
            Identity proof: {submission.documents.identityProofUri || '--'}
          </AppText>
          {submission.documents.identityProofUri ? (
            <Image source={{ uri: submission.documents.identityProofUri }} style={styles.previewImage} resizeMode="cover" />
          ) : null}
